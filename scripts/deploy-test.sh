...
ZIP_PATH="/tmp/release-test.zip"
EXTRACT_DIR="/tmp/release-test-$$"
RELAY_BIN="/opt/relay-test/bin/relay-server"
RELAY_WEB="/opt/relay-test/web/dist"
RELAY_USER="relay"
SERVICE="relay-test.service"
PORT="8081"
...
sudo chown -R "$RELAY_USER:$RELAY_USER" /opt/relay-test
...
if [ "$HTTP_CODE" != "200" ]; then
    echo "Deploy finished but HTTP $HTTP_CODE — check logs: sudo journalctl -u $SERVICE -f"
    exit 1
fi
...
curl -s -X POST -H "Authorization: Bearer $TOKEN" \
  http://127.0.0.1:$PORT/api/admin/sources/s_001/check > /dev/null
